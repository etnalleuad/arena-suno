...
ROOT = Path('/home/user')
source = ROOT / 'suno_knowledge_base_ru_2026-09-06.md'
...
css = '''
:root{--ink:#242726;--muted:#68716d;--accent:#bd5032;--paper:#fffefa;--page:#f3f3ed;--line:#dedfd5;--nav:#202824;--navtext:#d7ded9;--code:#edf1eb;--soft:#e8eee5}
...
html=f'''<!DOCTYPE html>
<html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Практическая база знаний Suno: Lyrics Engineering, Style, Studio 2.0, Premier и предстоящий переход к v6. Срез 6 сентября 2026."><title>Suno — база знаний продюсера · 06.09.2026</title><style>{css}</style></head>
<body><div class="progress" aria-hidden="true"></div><aside><div class="brand"><small>PRODUCER FIELD MANUAL</small><b>SUNO /</b><p>Практическая база знаний</p><span class="stamp">Срез · 06 сентября 2026</span></div><nav aria-label="Содержание">{''.join(nav)}</nav><div class="sidebar-foot">Официальные функции ≠ вероятностные приёмы. Проверяйте гипотезы, сохраняйте удачные дубли.</div></aside>
...
<header class="hero"><div class="eyebrow">LYRICS · SOUND DESIGN · PRODUCTION</div><h1>От идеи —<br>к управляемому треку.</h1><p class="deck">Тексты, вокальная режиссура, архитектура Style и работа в Studio 2.0. Подтверждённые возможности отдельно от мифов о «секретных тегах».</p><div class="metrics"><div><strong>03 блока</strong>единая производственная система</div><div><strong>05 промптов</strong>готовые жанровые отправные точки</div><div><strong>{count_sources} источников</strong>официальные материалы и сообщества</div><div><strong>v5.5 → v6</strong>подтверждённое и предстоящее</div></div></header>
<main class="article">{soup}</main><footer class="endnote">Документальный ресерч · 6 сентября 2026. Страница самодостаточна: поиск, содержание и примеры работают без внешних библиотек. Ссылки на источники требуют доступа к сети. Для переноса в базу знаний используйте сопровождающий Markdown-файл.</footer></div><div id="toast" role="status"></div><script>{js}</script></body></html>'''
out=ROOT/'suno_knowledge_base_ru_2026-09-06.html'
out.write_text(html)
...
assert len(soup.find_all('pre'))==36
